...
def write_datafile(filename, toks):
    ...
    print(f"\nwriting {toks.shape} tokens to {filename}")
...
def tokenize(doc, tokenizer, max_length):
    ...
    # seq classification format:
    # immediately after CLS add label_idx + uint16 max
    label_fmt = [doc["label"], np.iinfo(np.uint16).max]
    seq = np.insert(seq, 1, label_fmt)
    return seq


def tokenize_mnli(ds, split, max_length=2**16):
    ...
    with mp.Pool(nprocs) as pool:
        ...
        for tokens in pool.imap(tokenize_fn, ds, chunksize=16):
            # Update progress bar
            ...
            if current_size + len(tokens) > args.shard_size and current_size > 0:
                ...
                all_tokens_np = np.concatenate(current_shard, axis=-1)
                filename = os.path.join(DATA_CACHE_DIR, f"mnli_{split}_{shard_index:06d}.bin")
            ...
            if progress_bar:
                progress_bar.update(tokens.size)
...
parser = argparse.ArgumentParser(description="MNLI dataset preprocessing")
...
if __name__ == "__main__":
    ...
    for split in ["validation_matched", "validation_mismatched", "test_matched", "test_mismatched", "train"]:
        ds = load_dataset("nyu-mll/glue", "mnli", split=split, streaming=True)
        tokenize_mnli(ds, split=split, max_length=args.max_length)
